package com.hillel.homework.hw13.library;

import com.hillel.homework.hw13.library.enums.*;
import com.hillel.homework.hw13.library.music.Sound;
import com.hillel.homework.hw13.library.objects.*;

import java.io.*;
import java.util.List;

import static com.hillel.homework.hw13.library.ExtraMethods.*;
import static com.hillel.homework.hw13.library.objects.LibraryObjects.*;

public class Library {
    static BufferedReader READER = new BufferedReader(new InputStreamReader(System.in));
    static String FILE_NAME = "src/com/hillel/homework/hw13/library/libraryObjects.bin";
    ...
    private static void save() throws IOException {
        ...
        try (FileOutputStream fos = new FileOutputStream(file);
            ...
            for (Author listItem : authors) {
                oos.writeObject(listItem);
            }
            oos.writeInt(books.size());
            for (Book listItem : books) {
                oos.writeObject(listItem);
            }
            oos.writeInt(genres.size());
            for (Genre listItem : genres) {
                oos.writeObject(listItem);
            }
    ...
    private static void music() {
        try {
            if (askUser("Do you want to listen classical music while working? Enter yes/ press any key to continue in silence")) {
                ...
                String path = "src/com/hillel/homework/hw13/library/music/";
                File file;
                switch (readNumber()) {
                    case 1:
